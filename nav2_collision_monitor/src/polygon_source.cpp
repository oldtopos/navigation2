  ...
  std::vector<Point> & data) const
{
  ...
  for (const auto & polygon_instance_stamped : data_) {
    if (base_shift_correction_) {
      // Obtaining the transform to get data from source frame and time where it was received
      // to the base frame and current time
      if (
        !nav2_util::getTransform(
          polygon_instance_stamped.header.frame_id, polygon_instance_stamped.header.stamp,
      ...
      if (
        !nav2_util::getTransform(
          polygon_instance_stamped.header.frame_id, base_frame_id_,
          transform_tolerance_, tf_buffer_, tf_transform))
      {
        return false;
      }
    ...
    geometry_msgs::msg::PolygonStamped poly_in;
    poly_in.header = polygon_instance_stamped.header;

    poly_in.polygon.points.reserve(polygon_instance_stamped.polygon.polygon.points.size());
    poly_in.polygon.points = polygon_instance_stamped.polygon.polygon.points;

    geometry_msgs::msg::PolygonStamped poly_out;
    geometry_msgs::msg::TransformStamped tf = tf2::toMsg(tf_transform);
    tf2::doTransform(poly_in, poly_out, tf);
...
void PolygonSource::dataCallback(geometry_msgs::msg::PolygonInstanceStamped::ConstSharedPtr msg)
{
  ...
  data_.erase(
    std::remove_if(
      ...
      [this,
      curr_time](const geometry_msgs::msg::PolygonInstanceStamped & polygon_instance_stamped) {
        return curr_time - rclcpp::Time(polygon_instance_stamped.header.stamp) > source_timeout_;
      }), data_.end());

  // check if older similar polygon exists already and replace it with the new one
  for (auto & polygon_instance_stamped : data_) {
    if (msg->header.frame_id == polygon_instance_stamped.header.frame_id &&
      rclcpp::Time(msg->header.stamp) >= rclcpp::Time(polygon_instance_stamped.header.stamp) &&
      msg->polygon.id == polygon_instance_stamped.polygon.id)
    {
      polygon_instance_stamped = *msg;
      return;
    }
  }
  data_.push_back(*msg);
